 ...
 * @param isCore
 */
function buildShader(filePath, basePackageName = "core", isCore) {
    ...
    if ((0, utils_js_1.checkArgs)("--global", true)) {
...
export const ${shaderName + (isWGSL ? "WGSL" : "")} = { name, shader };`);
    ...
    (0, utils_js_1.checkDirectorySync)(path.dirname(tsShaderFilename));
    // check hash
    if (fs.existsSync(tsShaderFilename)) {
        const hash = (0, utils_js_1.getHashOfFile)(tsShaderFilename);
        const newHash = (0, utils_js_1.getHashOfContent)(tsContent);
        if (hash === newHash) {
            return;
        }
    }
