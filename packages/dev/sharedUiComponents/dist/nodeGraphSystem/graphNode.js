...
export class GraphNode {
        ...
        this._onSelectionChangedObserver = this._stateManager.onSelectionChangedObservable.add((options) => {
            ...
            if (node === this) {
                ...
                if (this._ownerCanvas.selectedNodes.indexOf(this) === -1) {
                    this._visual.classList.remove(localStyles["selected"]);
                    if (this._displayManager && this._displayManager.onSelectionChanged) {
                        this._displayManager.onSelectionChanged(this.content, node && node.content ? node.content : null, this._stateManager);
                    }
                }
            }
        });
    ...
    _forceRebuild(source, propertyName, notifiers) {
        for (const refresh of this._visualPropertiesRefresh) {
            refresh();
        }
                ...
                switch (type) {
                    ...
                    case 1 /* PropertyTypeForEdition.Float */: {
                        ...
                        if (cantDisplaySlider) {
                            ...
                            slider.step = type === 2 /* PropertyTypeForEdition.Int */ ? "1" : (Math.abs(options.max - options.min) / 100.0).toString();
