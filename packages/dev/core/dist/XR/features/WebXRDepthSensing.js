...
export class WebXRDepthSensing extends WebXRAbstractFeature {
    ...
    _updateDepthInformationAndTextureCPUDepthUsage(frame, view, dataFormat) {
        ...
        if (!this._cachedDepthImageTexture) {
            this._cachedDepthImageTexture = RawTexture.CreateRTexture(null, width, height, this._xrSessionManager.scene, false, true, Texture.NEAREST_SAMPLINGMODE, Constants.TEXTURETYPE_FLOAT);
        }
        switch (dataFormat) {
            case "ushort":
    ...
    _updateDepthInformationAndTextureWebGLDepthUsage(webglBinding, view, dataFormat) {
        ...
        if (!this._cachedDepthImageTexture) {
            this._cachedDepthImageTexture = RawTexture.CreateRTexture(null, width, height, scene, false, true, Texture.NEAREST_SAMPLINGMODE, dataFormat === "ushort" ? Constants.TEXTURETYPE_UNSIGNED_BYTE : Constants.TEXTURETYPE_FLOAT);
        }
