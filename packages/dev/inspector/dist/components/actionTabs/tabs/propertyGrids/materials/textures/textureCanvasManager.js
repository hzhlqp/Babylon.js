...
export class TextureCanvasManager {
    ...
    async grabOriginalTexture() {
        ...
        const data = await TextureHelper.GetTextureDataAsync(this._originalTexture, this._size.width, this._size.height, this._face, { R: true, G: true, B: true, A: true }, undefined, this._mipLevel);
        ...
        Tools.ReadFile(file, (data) => {
            ...
            reader.onloadend = () => {
                ...
                if (extension === ".dds" || extension === ".env") {
                    this._originalTexture.updateURL(base64data, extension, () => this.grabOriginalTexture());
                }
                else {
                    const texture = new Texture(base64data, this._scene, this._originalTexture.noMipmap, false, Texture.NEAREST_SAMPLINGMODE, () => {
                        TextureHelper.GetTextureDataAsync(texture, texture.getSize().width, texture.getSize().height, 0, { R: true, G: true, B: true, A: true }).then(async (pixels) => {
                            if (this._tool && this._tool.instance.onReset) {
                                this._tool.instance.onReset();
                            }
                            texture.dispose();
...
//# sourceMappingURL=textureCanvasManager.js.map