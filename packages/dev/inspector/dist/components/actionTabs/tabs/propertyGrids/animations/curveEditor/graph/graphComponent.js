...
export class GraphComponent extends React.Component {
    constructor(props) {
        ...
        this._graphAbsoluteHeight = 357;
        this._viewWidth = 788;
        this._viewCurveWidth = 788;
        this._viewHeight = 357;
        this._viewScale = 1;
        this._offsetX = 0;
        this._offsetY = 0;
        this._graphOffsetX = 30;
        this.state = {};
        ...
        this.props.context.onCreateOrUpdateKeyPointRequired.add(() => {
            ...
            for (const currentAnimation of this.props.context.activeAnimations) {
                ...
                if (this.props.context.target) {
                    value = this.props.context.target;
    ...
    _setDefaultInTangent(keyId) {
        for (const curve of this._curves) {
            curve.storeDefaultInTangent(keyId);
        }
    }
    _setDefaultOutTangent(keyId) {
        for (const curve of this._curves) {
            curve.storeDefaultOutTangent(keyId);
        }
    }
    _evaluateKeys(frame = true, range = true) {
        if (this.props.context.activeAnimations.length === 0) {
            this._curves = [];
            return;
        }
    ...
    _extractValuesFromKeys(keys, dataType, curvesToPushTo, propertyFilter) {
    ...
    _buildFrameIntervalAxis() {
        ...
        return steps.map((s) => {
            ...
            return (_jsx("g", { children: _jsx("line", { x1: x, y1: 0, x2: x, y2: this._viewHeight * this._viewScale, style: {
                        stroke: "#666666",
                        strokeWidth: 1,
                    } }, "line" + s) }, "axis" + s));
        });
    ...
    _buildYAxis() {
        ...
        return steps.map((s) => {
            ...
            return (_jsxs("g", { children: [_jsx("line", { x1: this._graphOffsetX * this._viewScale, y1: y, x2: this._viewWidth * this._viewScale, y2: y, style: {
                            stroke: isZero ? "#666666" : "#333333",
                            strokeWidth: isZero ? 1.0 : 0.5,
                        } }, "line" + s), _jsx("text", { x: 0, y: y, dx: `${15 * this._viewScale}px`, textAnchor: "middle", dy: `${3 * this._viewScale}px`, style: {
                            fontFamily: "acumin-pro-condensed",
                            fontSize: `${10 * this._viewScale}px`,
                            fill: "#888888",
                            textAlign: "center",
                        }, children: text }, "label" + s)] }, "axis" + s));
        });
    }
    _frameFromActiveKeys() {
        if (this.props.context.activeAnimations.length === 0) {
            return;
        }
        this._offsetX = 20;
    ...
    _dropKeyFrames(curve) {
        return curve.keys.map((key, i) => {
            const x = this._convertX(key.frame);
            const y = this._convertY(key.value);
            return (_jsx(KeyPointComponent, { x: x, y: y, context: this.props.context, scale: this._viewScale, getPreviousX: () => (i > 0 ? this._convertX(curve.keys[i - 1].frame) : null), getNextX: () => (i < curve.keys.length - 1 ? this._convertX(curve.keys[i + 1].frame) : null), channel: curve.color, keyId: i, curve: curve, invertX: (x) => this._invertX(x), invertY: (y) => this._invertY(y), convertX: (x) => this._convertX(x), convertY: (y) => this._convertY(y), onFrameValueChanged: (value) => {
    ...
    render() {
        ...
        return (_jsxs("div", { id: "graph", onWheel: (evt) => this._onWheel(evt), onPointerDown: (evt) => this._onPointerDown(evt), onPointerMove: (evt) => this._onPointerMove(evt), onPointerUp: (evt) => this._onPointerUp(evt), children: [this.props.context.activeAnimations.length !== 0 && (_jsx("div", { id: "dark-rectangle", style: {
                        left: activeBoxLeft + "px",
                        width: activeBoxRight - activeBoxLeft + "px",
                    } })), _jsx("div", { id: "block-rectangle" }), _jsx("svg", { id: "svg-graph-grid", viewBox: viewBoxScalingGrid, ref: this._svgHost, children: this._buildYAxis() }), _jsx("svg", { id: "svg-graph-horizontal", viewBox: viewBoxHorizontal, children: this._buildFrameIntervalAxis() }), _jsxs("svg", { ref: this._svgHost2, id: "svg-graph-curves", tabIndex: 0, viewBox: viewBoxScalingCurves, children: [this._curves.map((c, i) => {
                            return _jsx(CurveComponent, { context: this.props.context, curve: c, convertX: (x) => this._convertX(x), convertY: (y) => this._convertY(y) }, i);
                        }), this._curves.map((c) => {
                            return this._dropKeyFrames(c);
                        })] }), _jsx("div", { ref: this._selectionRectangle, id: "selection-rectangle" })] }));
    }
}
