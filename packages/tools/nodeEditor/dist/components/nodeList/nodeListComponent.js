...
export class NodeListComponent extends React.Component {
    ...
    render() {
        ...
        for (const key in allBlocks) {
            const blockList = allBlocks[key]
                ...
                if (key === "Custom_Frames") {
                    return (_jsx(DraggableLineWithButtonComponent, { format: "babylonjs-material-node", data: block, tooltip: this._customFrameList[block] || "", iconImage: deleteButton, iconTitle: "Delete", onIconClick: (value) => this.removeItem(value) }, block));
                }
                else if (key === "Custom_Blocks") {
                    return (_jsx(DraggableLineWithButtonComponent, { format: "babylonjs-material-node", data: block, tooltip: this._customBlockList[block] || "", iconImage: deleteButton, iconTitle: "Delete", onIconClick: (value) => this.removeCustomBlock(value), lenSuffixToRemove: 11 }, block));
                }
                return _jsx(DraggableLineComponent, { format: "babylonjs-material-node", data: block, tooltip: NodeListComponent._Tooltips[block] || "" }, block);
            });
            if (key === "Custom_Frames") {
                const line = (_jsx(LineWithFileButtonComponent, { title: "Add Custom Frame", closed: false, multiple: true, label: "Add...", uploadName: "custom-frame-upload", iconImage: addButton, accept: ".json", onIconClick: (file) => {
                        this.loadCustomFrame(file);
                    } }, "add..."));
                blockList.push(line);
            }
            else if (key === "Custom_Blocks") {
                const line = (_jsx(LineWithFileButtonComponent, { title: "Add Custom Block", closed: false, multiple: true, label: "Add...", uploadName: "custom-block-upload", iconImage: addButton, accept: ".json", onIconClick: (file) => {
        ...
        return (_jsx("div", { id: "nmeNodeList", children: _jsx("div", { className: "panes", children: _jsxs("div", { className: "pane", children: [_jsx("div", { className: "filter", children: _jsx("input", { type: "text", placeholder: "Filter", onFocus: () => (this.props.globalState.lockObject.lock = true), onBlur: () => {
