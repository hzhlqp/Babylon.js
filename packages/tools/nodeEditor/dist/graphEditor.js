...
export class GraphEditor extends React.Component {
    appendBlock(dataToAppend, recursion = true) {
        return this._graphCanvas.createNodeFromObject(dataToAppend instanceof NodeMaterialBlock ? TypeLedger.NodeDataBuilder(dataToAppend, this._graphCanvas) : dataToAppend, (block) => {
    ...
    constructor(props) {
        ...
        this.props.globalState.onImportFrameObservable.add((source) => {
            ...
            this.props.globalState.nodeMaterial.attachedBlocks.slice(-frameData.blocks.length).forEach((block) => {
    ...
    loadGraph() {
        const material = this.props.globalState.nodeMaterial;
        material._vertexOutputNodes.forEach((n) => {
            this.appendBlock(n, true);
        });
        material._fragmentOutputNodes.forEach((n) => {
            this.appendBlock(n, true);
        });
        material.attachedBlocks.forEach((n) => {
            this.appendBlock(n, true);
        });
        // Links
        material.attachedBlocks.forEach((n) => {
    ...
    resizeColumns(evt, forLeft = true) {
        ...
        const rootElement = evt.currentTarget.ownerDocument.getElementById("node-editor-graph-root");
        if (forLeft) {
    ...
    emitNewBlock(blockType, targetX, targetY) {
        ...
        if (blockType.indexOf("Block") === -1) {
            ...
            block.autoConfigure(this.props.globalState.nodeMaterial, (filterBlock) => !this._graphCanvas.nodes.some((node) => node.enclosingFrameId >= 0 && node.content.data.uniqueId === filterBlock.uniqueId));
    ...
    render() {
        return (_jsxs(Portal, { globalState: this.props.globalState, children: [_jsxs("div", { id: "node-editor-graph-root", style: {
                        ...
                        this.props.globalState.lockObject.lock = false;
                                ...
                                } }) }), _jsx("div", { id: "rightGrab", onPointerDown: (evt) => this.onPointerDown(evt), onPointerUp: (evt) => this.onPointerUp(evt), onPointerMove: (evt) => this.resizeColumns(evt, false) }), _jsxs("div", { className: "nme-right-panel", children: [_jsx(PropertyTabComponent, { lockObject: this.props.globalState.lockObject, globalState: this.props.globalState }), !this.state.showPreviewPopUp ? _jsx(PreviewMeshControlComponent, { globalState: this.props.globalState, togglePreviewAreaComponent: this.handlePopUp }) : null, !this.state.showPreviewPopUp ? _jsx(PreviewAreaComponent, { globalState: this.props.globalState, width: this._rightWidth }) : null] }), _jsx(LogComponent, { globalState: this.props.globalState })] }), _jsx(MessageDialog, { message: this.state.message, isError: this.state.isError, onClose: () => this.setState({ message: "" }) }), _jsx("div", { className: "blocker", children: "Node Material Editor runs only on desktop" }), _jsx("div", { className: "wait-screen hidden", children: "Processing...please wait" })] }));
    }
}
