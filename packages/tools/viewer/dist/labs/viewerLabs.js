...
export class ViewerLabs {
    ...
    loadEnvironment(data, onSuccess, onProgress, onError) {
        ...
        if (data instanceof ArrayBuffer) {
            ...
            this._scene._loadFile(url, (arrayBuffer) => {
     ...
     * @param rotationY
     */
    applyEnvironmentMapConfiguration(rotationY) {
        if (!this.environment) {
            return;
        }
        //set orientation
    ...
    rotateShadowLight(shadowLight, amount, point = Vector3.Zero(), axis = Axis.Y, target = Vector3.Zero()) {
        axis.normalize();
