...
import { Animation, CircleEase, BackEase, BounceEase, CubicEase, ElasticEase, ExponentialEase, PowerEase, QuadraticEase, QuarticEase, QuinticEase, SineEase, } from "core/Animations/index";
...
export class ViewerModel {
    constructor(_observablesManager, modelConfiguration, _configurationContainer) {
    ...
    get shadowsRenderedAfterLoad() {
        return this._shadowsRenderedAfterLoad;
    }
    set shadowsRenderedAfterLoad(rendered) {
        if (!rendered) {
    ...
    _configureModel() {
        ...
        const meshesWithNoParent = [this.rootMesh]; //this._meshes.filter(m => m.parent === this.rootMesh);
        const updateMeshesWithNoParent = (variable, value, param) => {
        ...
        const updateXYZ = (variable, configValues) => {
        ...
        if (this._modelConfiguration.rotationOffsetAxis) {
            const rotationAxis = new Vector3(this._modelConfiguration.rotationOffsetAxis.x, this._modelConfiguration.rotationOffsetAxis.y, this._modelConfiguration.rotationOffsetAxis.z);
    ...
    _modelAnimationConfigurationToObject(animConfig) {
        const anim = {
            time: 0.5,
        };
     ...
     * @internal
     */
    _applyModelMaterialConfiguration(material) {
     ...
     */
    _applyAnimation(animationConfiguration, isEntry, completeCallback) {
        const animations = [];
        //scale
        if (animationConfiguration.scaling) {
            const scaleStart = isEntry ? animationConfiguration.scaling : new Vector3(1, 1, 1);
            const scaleEnd = isEntry ? new Vector3(1, 1, 1) : animationConfiguration.scaling;
     ...
     */
    transitionTo(animations, duration, easingFunction, easingMode = 2, // BABYLON.EasingFunction.EASINGMODE_EASEINOUT,
    onAnimationEnd) {
        if (easingFunction) {
        ...
        if (this.rootMesh.getScene().beginAnimation !== undefined) {
            const animatable = this.rootMesh.getScene().beginAnimation(this.rootMesh, 0, this._frameRate * duration, false, 1, () => {
                if (onAnimationEnd) {
                    onAnimationEnd();
                }
            });
     ...
     */
    _setLinearKeys(animation, startValue, endValue, duration) {
        animation.setKeys([
            {
                frame: 0,
                value: startValue,
     ...
     */
    _createEasingFunction(easingFunctionID) {
        let easingFunction;
