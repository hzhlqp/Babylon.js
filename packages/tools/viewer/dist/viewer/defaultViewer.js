...
export class DefaultViewer extends AbstractViewerWithTemplate {
     ...
     */
    constructor(containerElement, initialConfiguration = { extends: "default" }) {
        super(containerElement, initialConfiguration);
        this.containerElement = containerElement;
        this._registeredPlugins = [];
        this._handlePointerClick = (event) => {
         ...
         * @param noUiUpdate
         */
        this._togglePlayPause = (noUiUpdate) => {
            if (!this._currentAnimation) {
                return;
            }
        ...
        this._updateProgressBar = () => {
            ...
            const progressSlider = navbar.parent.querySelector("input.progress-wrapper");
         ...
         * @param paramsObject
         */
        this._updateAnimationSpeed = (speed, paramsObject) => {
            const navbar = this.templateManager.getTemplate("navBar");
            if (!navbar) {
                return;
            }
         ...
         * @param paramsObject
         */
        this._updateAnimationType = (data, paramsObject) => {
            const navbar = this.templateManager.getTemplate("navBar");
            if (!navbar) {
                return;
            }
        ...
        this.onEngineInitObservable.add(() => {
            ...
            this.sceneManager.setDefaultMaterial = function (sceneConfig) {
    ...
    registerTemplatePlugin(plugin) {
        ...
        if (plugin.eventsToAttach) {
            plugin.eventsToAttach.forEach((eventName) => {
                plugin.onEvent &&
                    this.templateManager.eventManager.registerCallback(plugin.templateName, (event) => {
        ...
        if (this.configuration.templates && this.configuration.templates.viewer) {
            if (this.configuration.templates.viewer.params && this.configuration.templates.viewer.params.enableDragAndDrop) {
                this.onSceneInitObservable.addOnce(() => {
                    const filesInput = new FilesInput(this.engine, this.sceneManager.scene, () => { }, () => { }, () => { }, () => { }, function () { }, (file) => {
                        this.loadModel(file);
                    }, () => { });
    ...
    _initNavbar() {
        ...
        if (navbar) {
            ...
            this.templateManager.eventManager.registerCallback("navBar", (event) => {
    ...
    _prepareContainerElement() {
        const htmlElement = this.containerElement;
     ...
     */
    _configureTemplate(model) {
        const navbar = this.templateManager.getTemplate("navBar");
        if (!navbar) {
            return;
        }
        const newParams = navbar.configuration.params || {};
        if (!model) {
        ...
        return template.hide((template) => {
            ...
            if (overlays) {
                for (let i = 0; i < overlays.length; ++i) {
                    const htmlElement = overlays.item(i);
    ...
    showLoadingScreen() {
        ...
        return template.show((template) => {
            ...
            if (this.configuration.templates && this.configuration.templates.loadingScreen) {
                color = (this.configuration.templates.loadingScreen.params && this.configuration.templates.loadingScreen.params.backgroundColor) || color;
            }
