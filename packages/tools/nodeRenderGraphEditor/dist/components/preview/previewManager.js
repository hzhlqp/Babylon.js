...
export class PreviewManager {
    ...
    _initScene(scene) {
        window.scenePreview = scene;
        this._scene = scene;
        this._globalState.filesInput?.dispose();
        this._globalState.filesInput = new FilesInput(this._engine, null, (_, scene) => {
    ...
    _createNodeRenderGraph() {
        ...
        window.nrgPreview = this._nodeRenderGraph;
    }
    async _buildGraph() {
        if (!this._scene) {
            // The initialization is not done yet
        ...
        const guiBlocks = this._nodeRenderGraph.getBlocksByPredicate((block) => block.getClassName() === "GUI.NodeRenderGraphGUIBlock");
        let guiIndex = 0;
    ...
    _frameCamera() {
        ...
        for (const camera of this._scene.cameras) {
            const arcRotateCamera = camera;
            const framingBehavior = arcRotateCamera.getBehaviorByName("Framing");
